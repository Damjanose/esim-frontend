...
## Current state
- App Router structure under `src/app/`: public marketing pages (`destinations` browse hub, `esim/[slug]` destination SEO, `travel` + `travel/[slug]`, `use-cases` + `use-cases/[slug]`, `compare` + `compare/[slug]`, `support`, `policy`, `terms`), auth (`signin`), account/purchase (`account`, `account/[orderId]`, `account/topup`, `profile`, `profile/billing`, `profile/deleted`), and `checkout` + `checkout/return` + `checkout/failed`. Legacy `/destinations/:slug` and `/guides/:slug` 301 to the new paths.
- Admin surfaces live under deliberately obfuscated route names, not `/admin*`: `xloginy` (admin sign-in), `xpricing` (price management), `xnotificationy` (broadcast drafts plus an Individual tab for one-off per-user pushes, send history, and force-logout of unlinked or all accounts), `xerrors`, `xversion` (app version/usage report), `xactivityy`, `xpartnersy`, `xsupport` (live support inbox), `xtripplany` (trip-plan provider, write-only API key, rolling generation quota, USD download price — traveler itinerary UI is mobile-only) — obfuscation is the access control, don't rename these to anything guessable
...
- BFF pattern: all **REST** backend calls are proxied through `src/app/bff/*` Next.js route handlers, never called from client components directly — keeps backend base URL/credentials server-side. Socket.IO from `/xsupport` is the only browser→Express channel (admin JWT in the handshake).
